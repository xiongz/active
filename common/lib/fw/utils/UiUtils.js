...
define(function () {
    ...
    utils.showLoading = function () {
    	if($(".gtb_max_loading").length>0){return;}
		var $div = '<div class="gtb_max_loading" style="width:100%;height:100%;position:fixed;top:0;left:0;z-index:11000;display: none;"><img src="common/images/loading/loading.gif" style="width:30%; display:block;position: absolute;top: 40%;left:35%;"></div>';
        $($div).appendTo($("body")).fadeIn();
    }
    utils.hideLoading = function () {
		$(".gtb_max_loading").fadeOut().remove();
    }

    utils.msg = function (title, msgText) {
     ...
     */
    utils.confirmMSG = function (title, msgText, okText, cancelText, confirmcallback) {

    }

    utils.reWindowUI = function(){
        var Showbo={author:'xiongze',homepage:''};
    //是否为ie浏览器
    ...
    //页面的高和宽******************************
        Showbo.Msg={
            ...
            alert:function(msg){
                Showbo.Msg.show({buttons:{yes:'确认'},msg:msg,title:'消息'});
            },
            confirm:function(msg,title,btns,fn){
                //fn为回调函数，参数和show方法的一致
                Showbo.Msg.show({buttons:{yes:btns[0]||"确认",no:btns[1]||"取消"},msg:msg,title:title,fn:fn});
            },
            prompt:function(labelWord,defaultValue,txtId,fn){
                if(!labelWord)labelWord='请输入：';
                if(!defaultValue)defaultValue="";
                if(!txtId)txtId="msg_txtInput";
                Showbo.Msg.show({title:'输入提示',msg:labelWord+'<input type="text" id="'+txtId+'" style="width:200px" value="'+defaultValue+'"/>',buttons:{yes:'确认',no:'取消'},fn:fn});
            },
        ...
        window.alert = Showbo.Msg.alert;
        window.confirm = Showbo.Msg.confirm;
        window.prompt = Showbo.Msg.prompt;
    }
